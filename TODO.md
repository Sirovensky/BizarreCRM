...
- [ ] OPS-DEFERRED-001. **Multi-platform setup migration (`setup.bat` → `setup.mjs`) + cross-platform auto-startup adapter.**
  - [x] **Phase 0 LANDED 2026-05-05**: per-OS gateway shims (`setup.bat` + `setup.command` + `setup.sh`) verify Node v22+ and best-effort install via winget / Homebrew / apt-dnf-yum-pacman-zypper-apk-NodeSource, falling back to opening `https://nodejs.org/en/download/` on any failure. Gateways exec `setup.mjs`. Existing Windows-only flow preserved at `scripts/setup-windows.bat`; `setup.mjs` delegates there on `process.platform === 'win32'` and prints a manual-step list on darwin/linux. Bootstrap question resolved: gateways can install Node, OR direct the operator to install it.
  - [ ] BLOCKED Phase 1+: porting the Windows-only `scripts/setup-windows.bat` logic (build, env, certs, PM2 launch, dashboard) into cross-platform Node code in `setup.mjs`. Requires: deciding the autostart approach per OS (`scripts/autostart/{index,linux,darwin,win32}.mjs` with thin per-OS adapters: Linux/macOS → `pm2 startup` + `pm2 save`; Windows → Task Scheduler XML via `schtasks` — NO vendored binaries, NO PowerShell scripts).
  - Acceptance when fully unblocked: fresh boot on Linux/macOS/Windows brings CRM online without user login; zero `process.platform === 'win32'` branches outside the three adapter files; `scripts/setup-windows.bat` deleted.
