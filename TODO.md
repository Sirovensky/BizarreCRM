...
- [ ] WEB-S4-020. **P1 — Portal Sign In phone has no formatting/strip — backend gets unstripped value.** Fix: strip non-digits before submit.
...
- [ ] WEB-FH-009. **[HIGH] InvoiceDetailPage credit-note cap is `total` not `amount_due` — refund can exceed actually-paid amount.** `InvoiceDetailPage.tsx:240` validates `amount > Number(invoice.total)` -> reject. So a $200 invoice that received only a $50 deposit can have a $200 "credit note" issued — refunding $150 the shop never collected. Server-side refunds.routes likely caps it but the UX promises operator "max=$200" and submits invalid amounts that bounce on race. Should cap at min(invoice.amount_paid, invoice.total - invoice.refunded_amount).
  <!-- meta: scope=web/pages/invoices; files=packages/web/src/pages/invoices/InvoiceDetailPage.tsx:237-247,694,700; fix=cap-creditNoteForm.amount-at-Number(invoice.amount_paid-invoice.refunded_amount)+update-placeholder-and-Max-label -->
...
- [ ] WEB-FH-014. **[MED] CheckoutModal split-payment validation accepts partial-cents underpay — splitTotal vs total comparison done in floats.** `CheckoutModal.tsx:180-186` `splitTotal` is built via `splitPayments.reduce((sum, sp) => sum + Math.round(parseFloat(sp.amount)*100), 0) / 100`. The compare at line 320 is `splitTotal < totals.total` — both floats. With 3 split lines each 33.33, splitTotal = 99.99 (text-truncated input), at exactly 99.99 < 100 -> blocks legitimate even-split when the cashier expects "Cover total $100". Should compare cents-int directly without re-dividing by 100.
  <!-- meta: scope=web/pages/unified-pos; files=packages/web/src/pages/unified-pos/CheckoutModal.tsx:180-186,319-323,384-386; fix=keep-splitTotalCents+totalCents-as-int+compare-int-only -->
...
- [ ] WEB-FX-012. **[LOW] Password show/hide toggle buttons (`LoginPage.tsx:584`, `SignupPage.tsx:345`) lack `aria-label` and `aria-pressed` — toggle button without state announcement.** Buttons render `<Eye>` / `<EyeOff>` icons with `tabIndex={-1}` (skip in tab order) but no `aria-label="Show password" / "Hide password"` and no `aria-pressed={showPassword}`. tabIndex=-1 also defeats keyboard discoverability.
