...
- [ ] **POS-SALES-001. Android `pos/sales` endpoint missing on server — tender finalization currently 404s.** Discovered 2026-04-24 during functional-parity audit. Android `PosApi.completeSale` POSTs to `/api/v1/pos/sales` (see `android/.../data/remote/api/PosApi.kt:92`) but `packages/server/src/routes/pos.routes.ts` only registers `/cash-in`, `/cash-out`, `/transaction`, `/checkout-with-ticket`, `/return`, `/open-drawer`, `/workstations`, `/workstations/:id/set-default`. The `/transaction` endpoint accepts inventory-only items (per inline comment line 343: "misc / custom items — not currently supported by this endpoint"); `/checkout-with-ticket` is a 1000-line ticket-aware path that requires `mode='checkout'` + `product_items` + `misc_items` separately. Neither matches Android's `{lines: PosCartLineDto[], cart_discount_cents, payment_amount_cents, payment_method}` shape. Tender screen calls `posApi.completeSale` from `PosTenderViewModel.finalizeSale` and surfaces `errorMessage = "Network error"` on every attempted sale. Plan: add a new `router.post('/sales', idempotent, asyncHandler(...))` that (1) accepts the Android cents-based schema, (2) translates internally (cents → dollars via `roundCents`, single `lines[]` → `items + custom items`), (3) reuses `validatePrice` / payment-method whitelist / `getOrCreateWalkInCustomerId` / atomic `adb.transaction` write of invoice + invoice_lines + payments + stock decrement. Should also accept multi-tender `payments[]` per AppliedTender list. Bound: ~150–200 server lines + integration test. Not started this wave to keep scope tight; flagged here so the next POS server pass picks it up. Blocking: real cart sales fail end-to-end on Android; only `Park cart` + `ACH stub` (no payment) currently complete the UI flow.
