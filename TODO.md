...
**Wave 5** (pending commit): Fixed SCAN-499 (recurringInvoices asyncHandler wrap ×7), SCAN-500 (shifts swap-accept atomic tx), SCAN-501 (next_run_at Date.parse validation), SCAN-502-504 (fieldService N+1 batched ×3), SCAN-505 (smsGroups /members rate-limit 10/min/user), SCAN-506 (activity metadata PII allowlist scrub). Started multi-location retrofit: migration 136 adds `tickets.location_id` column with backfill to main store id=1 + optional filter on GET/POST/PATCH (no forced scoping — backwards-compat). Scan-loop appended 14 new findings SCAN-507..520 (FK ON DELETE cascades, indices, heap-unbound queries, enum-bypass timing).
