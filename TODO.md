...
- [ ] CROSS-REPAIR-TYPE-TILES. **Tile/icon picker for repair types instead of plain text list (Android + Web + Server-seeded icons).** Directive 2026-04-23. Currently the ticket-create / quote flow renders repair types (screen replacement, battery swap, charge port, speaker, back-glass, water damage, software, diagnostics, etc.) as a scrollable text list — hard to scan on a small phone screen, still cramped on tablet. Replace with a compact grid of tappable tiles: icon + 1-2 word label + optional secondary text (e.g. starting price or ETA). Small tiles (roughly 80x80dp on phone, 96x96dp on tablet). Doesn't need to be fancy — just visual affordance + tap target.
  - [ ] **Android**: new `ui/components/RepairTypeTilePicker.kt` composable (FlowRow of Surface tiles + icon + label). Wire into TicketCreateScreen wizard step where repair type is picked. Tiles derived from server-seeded repair-service catalog (each service row gets an optional `icon` key — fallback to generic wrench icon). Respect ReduceMotion + contentDescription per tile ("Screen replacement, starting at $X, ETA Y hours").
  - [ ] **Web**: equivalent `<RepairTypeTileGrid />` component in `packages/web/src/pages/tickets/` used in ticket-create flow. Tailwind grid, responsive (2-col mobile, 4-col tablet, 6-col desktop). Same server-seeded catalog.
  - [ ] **Server**: extend `repair_services` schema with optional `icon_key` column (migration) seeded with common values (`screen`, `battery`, `charge_port`, `speaker`, `camera`, `water_damage`, `software`, `diagnostics`, `other`). Expose via existing `GET /repair-pricing/services`. Client maps icon_key → Material/Lucide icon.
  - Rationale: lots of text can be hard to bear especially on a smaller screen; visual tiles are faster to scan + tap. Low-effort UX win.
...
- [ ] SCAN-723. **[HIGH] estimates conversion rate-limit AFTER tier check — concurrent approvals both reserve tier quota before rate-limit rejects** — `packages/server/src/routes/estimates.routes.ts:969`. Fix: rate-limit FIRST, then tier check.
- [ ] SCAN-724. **[HIGH] estimate `converting` intermediate state has no finally-block rollback — tier-throw after lock acquires strands record** — `packages/server/src/routes/estimates.routes.ts:758-822`. Fix: try/finally to revert to prior status on throw.
- [ ] SCAN-725. **[HIGH] leads dry-run approval endpoint echoes customer PII (id/first_name/last_name/email/phone/mobile) — non-admin enumeration** — `packages/server/src/routes/leads.routes.ts:237`. Fix: restrict dry-run to manager+ OR redact PII in response.
...
- [ ] SCAN-727. **[HIGH] automations dry-run endpoint no rate-limit — brute-force trigger-rule enumeration** — `packages/server/src/routes/automations.routes.ts:200-295`. Fix: 20/min/user limit.
...
- [ ] SCAN-759. **[HIGH] tickets photo delete `path.join(config.uploadsPath, tenantSlug, photo.file_path)` — file_path from DB without normalize check; corrupted/injected value traverses** — `packages/server/src/routes/tickets.routes.ts:2360`. Fix: path.normalize + assert startsWith(tenant uploads root).
...
- [ ] SCAN-774. **[HIGH] inventory DELETE /:id missing requirePermission gate — destructive op** — `packages/server/src/routes/inventory.routes.ts`. Fix: `requirePermission('inventory.delete')`.
