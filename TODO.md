...
- [ ] WEB-FH-010. **[HIGH] CSV exports MISSING UTF-8 BOM — Excel mangles non-ASCII customer names + accented SKUs.** `InventoryListPage.tsx:309-310` and `CustomerListPage.tsx:322-323` both `new Blob([csv], { type: 'text/csv' })` — no leading BOM (U+FEFF), no `;charset=utf-8` in the mime. Open in Excel (Windows default code page CP-1252), accented Latin characters become mojibake, Chinese/Cyrillic become `??`. The shared `utils/csv.ts` toCsvRow doesn't add a BOM either. Affects every customer export to operators on Windows; tax accountants on Mac+Excel hit the same mojibake.
...
- [ ] WEB-FH-011. **[HIGH] CSV import parser uses naive .split(',') — quoted commas inside customer names blow up every imported row.** `CustomerListPage.parseImportCsv` line 339-353 + `InventoryListPage.parseImportCsv` line 320-331 both `lines[0].split(',')` and `line.split(',')`. A row like `"Smith, John",jsmith@x.com,...` splits into 4 columns instead of 3 — first_name=`"Smith`, last_name=`John"`, email=`jsmith@x.com`, phone=garbage. Round-trip export-then-import LOSES data because export properly quotes via toCsvField but import doesn't honor quoting. Real customers with commas in business names ("Smith, Jr Inc.") import broken.
...
- [ ] WEB-FJ-001. **[HIGH] Access token + super-admin token both stored in `localStorage` — any XSS reads them and the SA token grants tenant-impersonation across the whole estate.** `stores/authStore.ts:67,92,104,122` + `api/client.ts:298,307,323-324` + `pages/super-admin/TenantsListPage.tsx:67`. SUPER_ADMIN_TOKEN_KEY=`'superAdminToken'` sits in plain localStorage; once stolen, the attacker calls `POST /super-admin/api/tenants/{slug}/impersonate` and lands a tenant JWT for any shop. Industry guidance is to keep refresh + super-admin tokens in httpOnly cookies; only the short-lived access token may be in JS-reachable storage and only when CSP is strict. Code already keeps the refresh token cookie-side — close the loop on the SA path.
