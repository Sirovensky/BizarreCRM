...
- [ ] CROSS38b-notif. **Android Settings: Notification preferences sub-page** — Edit Profile row landed 2026-04-17. Still missing a Notifications preferences sub-page (separate from notifications inbox per CROSS54) plus optional Language + Terms/Privacy rows.

- [ ] CROSS47-seed. **Android ticket wizard: accept and pre-seed customerId from nav arg** — CROSS47 landed 2026-04-17 (primary "Create Ticket" CTA below Contact info on CustomerDetailScreen). The CTA currently routes to the wizard WITHOUT pre-seeding the customer because TicketCreateScreen ignores a customerId nav arg. Add optional `customerId` query arg to `Screen.TicketCreate` route + read in `TicketCreateViewModel` via `SavedStateHandle`, then seed `selectedCustomer`/skip step 1. Also covers the "Create Invoice", "Create Estimate" CTAs when those land.
...
- [ ] CROSS50-header-needs-api. **Android CustomerDetail quick-stats row — needs API plumbing (DEFERRED 2026-04-17):** The CROSS50 quick-stats row (ticket count, LTV, last visit) cannot be rendered from the existing `CustomerDetail` DTO (`packages/android/app/src/main/java/com/bizarreelectronics/crm/data/remote/dto/CustomerDto.kt`). The DTO carries `tickets: List<TicketListItem>?` and `invoices: List<InvoiceListItem>?` but NOT `ticket_count`, `lifetime_value`, `last_visit_at` as top-level fields. Server has the data — `GET /api/v1/customers/:id/analytics` (`packages/server/src/routes/customers.routes.ts:1288-1315`) returns `{ total_tickets, lifetime_value, avg_ticket_value, first_visit, last_visit, days_since_last_visit }`. To close: (1) add `CustomerAnalytics` DTO + `CustomerApi.getCustomerAnalytics(id)` wrapper, (2) have `CustomerDetailViewModel` kick off an analytics fetch in parallel with the detail fetch and expose both on UiState, (3) render the quick-stats row from the analytics response under the avatar. Estimated 30-45 min; deferred because the in-scope spike was a pure render change and plumbing a second endpoint into the detail flow is a separate concern.
