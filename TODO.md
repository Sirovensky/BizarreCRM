...
- [ ] WEB-S4-029. **P2 — No 20-photo cap enforcement.** `PhotoCapturePage.tsx:156-167`. Fix: disable add-more when length>=20.
...
- [ ] WEB-FI-004. **[HIGH] `client.ts` 5xx auto-toast (line 278-284) fires for EVERY failed mutation including ones with their own `onError` UI — dual toasts on each server error.** Components using `useMutation({ onError: (err) => toast.error(formatApiError(err)) })` already display a toast; the response interceptor unconditionally fires `toast.error('Server error...')` first. User sees two toasts stacked, or worse, a generic "Server error" that overrides a specific server message. Add an opt-out (e.g. `config.skipGlobal500Toast`) or convert to event-bus + opt-in instead.
...
- [ ] WEB-FI-020. **[LOW] `ErrorBoundary` (components/ErrorBoundary.tsx) `Reload Page` button does `setState({ hasError: false }) THEN window.location.reload()` — the setState is wasted (reload throws away React state) and the boundary briefly re-renders children with the failing tree before navigation.** `components/ErrorBoundary.tsx:28`. Drop the setState, just reload.
...
- [ ] WEB-FG-018. **[LOW] `LandingPage` LoginModal slug input uses `e.preventDefault()`-less `onKeyDown=e=>e.key==='Enter'&&handleGo()` — `Enter` while focused on the slug also bubbles to any parent listener / submits any wrapping form on a page that adds one later.** `LandingPage.tsx:110`. Minor today (no parent form), but combined with the modal having no `<form>` of its own, screen-reader users navigating with `Enter` get inconsistent UX vs. `<button>` with `type="submit"`. Also: input has no `<label>` — only `placeholder="yourshop"`, which is a WCAG 1.3.1 + 3.3.2 fail (placeholder-as-label).
...
- [ ] WEB-FH-023. **[LOW] EstimateDetailPage versions query refetches on every showVersions toggle — no staleTime, causes flicker on accordion expand-collapse.** Line 44-49: `enabled: showVersions` with no staleTime. Toggle accordion three times -> three full version-history fetches. Stable list of completed versions doesn't change minute-to-minute; should staleTime: 60_000.
...
- [ ] WEB-FH-024. **[LOW] GiftCardDetailPage uses local formatCurrency that strips sign via Math.abs — refund of -0 displays as +$0.00.** Lines 36-38: `function formatCurrency(amount){ return $${Math.abs(amount).toFixed(2)} }`. Then row line 294: `{tx.amount >= 0 ? '+' : '-'}{formatCurrency(tx.amount)}` — sign reapplied via prefix, but Math.abs trips -0 (+$0.00 displayed). Should use shared `@/utils/format.formatCurrency` (already imported elsewhere) and skip the local helper.
...
- [ ] WEB-FL-009. **[MED] /customer-portal registered twice — flat + wildcard — second route shadowed.** `App.tsx:377-378`: `<Route path="/customer-portal" element={<CustomerPortalPage />} />` AND `<Route path="/customer-portal/*" element={<CustomerPortalPage />} />`. The wildcard form already matches `/customer-portal` exactly, making the flat decl redundant; React Router won't error but the dup adds noise + a footgun if someone adds another `/customer-portal/something` route between them.
...
- [ ] WEB-FM-004. **[HIGH] `index.html` loads `Fredoka One` + `Inter` from Google Fonts but per `~/.claude/.../project_brand_fonts.md` brand fonts are Saved By Zero / Bebas Neue / Futura — every brand surface ships the wrong typography.** `packages/web/index.html` line ~32 loads `family=Fredoka+One&family=Inter:wght@300;400;500;600;700&family=JetBrains+Mono:...` and `tailwind.config.ts:58` declares `sans: ['Inter', ...]`. Mockups were corrected on 2026-04-24 to the canonical brand fonts but neither the HTML font-link nor Tailwind config were swapped. Net effect: every page on web ships the off-brand pair while logo files (Saved By Zero) sit unused. Already flagged in user memory as the "native + web code needs alignment" task.
...
- [ ] WEB-FX-003. **[HIGH] Only 13 of 59 modal-like overlays in `pages/` declare `role="dialog"` + `aria-modal="true"` — 46 (78%) are bare `<div className="fixed inset-0 ...">` overlays.** `grep -rn 'fixed inset-0' packages/web/src/pages | wc -l` = 59. Major flows missing it: `CustomerListPage.tsx:804` import modal, `CustomerDetailPage.tsx:565` membership edit, `LeadListPage.tsx:126`, `CalendarPage.tsx:93,204`, `MembershipSettings`, `RepairPricingTab`, `DeviceTemplatesPage`. Screen-reader users get no "dialog" announcement and no focus-trap — tab leaves modal back into background.
...
- [ ] WEB-FX-004. **[HIGH] Form-validation errors render `<p className="text-sm text-red-600 mt-1">` with NO `role="alert"` and NO `aria-live` — only 2 spots in entire pages tree use `role="alert"`.** `grep -rn 'role="alert"' packages/web/src/pages` returns 2 hits (`portal/components/PayNowButton.tsx:89`, `reports/TaxReportPage.tsx:73`). Every other field error in `LoginPage`, `SignupPage`, `CustomerCreatePage`, settings forms is a silent paragraph — error appears visually but screen reader does not announce. WCAG 3.3.1 violation.
