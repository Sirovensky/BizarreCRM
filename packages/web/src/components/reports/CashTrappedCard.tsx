...
export function CashTrappedCard() {
  ...
  if (isLoading) {
    return <div className="h-48 rounded-xl border border-gray-200 bg-gray-50 animate-pulse" />;
  }
  if (error || !data) {
    return <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-red-700 text-sm">Unavailable.</div>;
  }
  ...
  return (
    <div className="rounded-xl border-2 border-orange-300 bg-orange-50 p-4">
      ...
      <div className="mt-2 text-3xl font-black text-orange-900 tabular-nums">
      ...
      {data.top_offenders.length > 0 && (
        <ul className="mt-3 space-y-1 max-h-48 overflow-y-auto text-sm">
          {data.top_offenders.slice(0, 5).map(item => (
            <li key={item.id} className="flex justify-between bg-white/50 rounded px-2 py-1">
              <span className="truncate mr-2">{item.name}</span>
              <span className="flex-shrink-0 tabular-nums text-orange-800">
