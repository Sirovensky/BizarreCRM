...
export function ChurnAlert() {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4">
      ...
      {isLoading && <div className="h-32 bg-gray-50 rounded animate-pulse" />}
      {error && <div className="text-sm text-red-600">Failed to load</div>}

      {data && (
        <>
          <div className="text-3xl font-bold text-gray-900 tabular-nums">{data.at_risk_count}</div>
          ...
          <ul className="space-y-1 max-h-48 overflow-y-auto text-sm">
            {data.customers.slice(0, 10).map(c => (
              <li
                key={c.customer_id}
                className="flex justify-between gap-2 px-2 py-1 rounded hover:bg-gray-50"
              >
