...
function TrendIcon({ pct }: { pct: number }) {
  if (pct > 5) return <TrendingUp size={14} className="text-green-600" />;
  if (pct < -5) return <TrendingDown size={14} className="text-red-600" />;
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {data.forecast.slice(0, 10).map(f => (
              <tr key={f.category} className="border-b last:border-0">
