      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-5">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            {onBack && (
              <button aria-label="Go back" onClick={onBack} className="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 mt-0.5">
            ...
            <div>
              {ticket.customer_first_name && (
                <p className="text-sm text-gray-500 dark:text-gray-400">{ticket.customer_first_name}</p>
              )}
            </div>
          </div>
          <div className="text-right">
            <p className="text-lg font-bold text-gray-900 dark:text-gray-100">{ticket.order_id}</p>
            <p className="text-xs text-gray-400 dark:text-gray-500">{formatDate(ticket.created_at, locale)}</p>
          </div>
        </div>
        ...
        {ticket.devices.length > 0 && (
          <div className="rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-800/60 text-xs text-gray-500 dark:text-gray-400 border-b border-gray-100 dark:border-gray-800">
                ...
                {ticket.devices.map((d, i) => (
                  <tr key={i} className={i > 0 ? 'border-t border-gray-50 dark:border-gray-700' : ''}>
                    <td className="px-4 py-3">
                      <div className="font-medium text-gray-900 dark:text-gray-100">{d.service || d.name || d.type || 'Repair'}</div>
                      {d.name && d.service && (
                        <div className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{d.name}</div>
                      )}
                    </td>
                    <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">
                      {d.total != null ? formatCurrency(Number(d.total), currency, locale) : <span className="text-gray-300 dark:text-gray-600">&mdash;</span>}
                    </td>
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400 text-xs hidden sm:table-cell">
                      {d.notes || <span className="text-gray-300 dark:text-gray-600">&mdash;</span>}
                    </td>
                    <td className="px-4 py-3 text-right">
                      {d.status ? (
                        <span className="text-xs text-gray-600 dark:text-gray-300">{d.status}</span>
                      ) : (
        ...
        {ticket.invoice && (
          <div className="rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium text-gray-700 dark:text-gray-200">Invoice {ticket.invoice.order_id}</span>
            ...
            <div className="space-y-1 text-sm">
              ...
              <div className="flex justify-between font-semibold text-gray-900 dark:text-gray-100 pt-1 border-t border-gray-200 dark:border-gray-800">
        ...
        <div className="rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-4">
          <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-4">Timeline</h3>
          <div className="space-y-0">
            {fullTimeline.map((entry, i) => (
              <div key={i} className="flex gap-3">
                <div className="flex flex-col items-center">
                  <TimelineDot type={entry.type} />
                  {i < fullTimeline.length - 1 && <div className="w-px flex-1 bg-gray-200 dark:bg-gray-700" />}
                </div>
                <div className="pb-4 min-w-0 flex-1">
                  <p className="text-sm text-gray-800 dark:text-gray-200">{entry.description}</p>
                  {entry.detail && (
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 line-clamp-2">{entry.detail}</p>
                  )}
                  <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5">{formatDateTime(entry.created_at, locale)}</p>
                </div>
              </div>
            ))}
          </div>
