/**
 * PaymentLinksPage — list / create / cancel tokenized Stripe payment links.
 * §52 idea 1. Staff-facing at /billing/payment-links.
 *
 * Sends via SMS/email is not wired here — a "Copy link" button is enough
 * for day 1; SMS/email owner already has a send path in sms.routes.ts.
 */
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        >
          {showForm ? 'Close form' : 'New payment link'}
        </button>
      </div>
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <select
              value={form.provider}
              onChange={(e) =>
                setForm((f) => ({ ...f, provider: e.target.value as 'stripe' | 'blockchyp' }))
              }
              className="rounded-md border border-gray-300 px-3 py-2 text-sm"
            >
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={6} className="px-3 py-6 text-center text-gray-400">No payment links yet</td></tr>
            ) : (
              data.map((row) => (
