...
router.put('/prices/:id', async (req, res) => {
  ...
  await adb.run(`
    ...
    WHERE id = ?
  `, labor_price ?? null, default_grade ?? null, is_active ?? null, req.params.id);
...
router.get('/prices/:id/grades', async (req, res) => {
  const adb = req.asyncDb;
...
router.post('/prices/:id/grades', async (req, res) => {
  const adb = req.asyncDb;
  const priceExists = await adb.get('SELECT id FROM repair_prices WHERE id = ?', req.params.id);
  if (!priceExists) throw new AppError('Price not found', 404);
...
router.put('/adjustments', adminOnly, async (req, res) => {
  const db = req.db;
  const adb = req.asyncDb;
