...
- [~] Auto-save drafts every 2s to Room for ticket-create, customer-create, SMS-compose; never lost on crash/background. (commit 9fb71216 + c7dd6f5 — `DraftStore` + ticket-create 2s debounce shipped via `TicketCreateViewModel.onFieldChanged()` on every field; customer-create + SMS-compose wiring pending)
