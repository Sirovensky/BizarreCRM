...
- [x] **Status pie + payment-method pie** (tablet/ChromeOS) — pure-Canvas donut `InvoiceStatusPieChart` in `ui/screens/invoices/components/`; animated sweep (600 ms, ReduceMotion-aware); tappable legend with tooltip; tablet `InvoiceStatsHeader` shows pills column + chart `Row`; phone unchanged. (session 2026-04-27 — `InvoiceStatusPieChart.kt` 228 lines; payment-method pie NOTE-defer: server returns no per-method bucket breakdown)
