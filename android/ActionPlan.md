...
| 26 | Accessibility | ~44% | ReduceMotion util + Settings toggle + tests + BrandTopAppBar heading() semantics + BrandListItem mergeDescendants/Role.Button + DashboardScreen sweep (881b404) + TicketListScreen sweep (bf211de) + CustomerListScreen sweep (cda8fb0) + **InvoiceListScreen sweep (3260c15) + ExpenseListScreen sweep (9883b2e — summary liveRegion Polite + collapsible "By category" Role.Button + chip Role.Tab).** Missing: per-screen sweep across remaining domains (Estimates/Leads/Inventory/Appointments/SMS), fontScale, a11y framework tests. |
