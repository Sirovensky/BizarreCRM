...
public final class TwoFactorSettingsViewModel {
    ...
    public func regenerateCodes() async {
        ...
        do {
            ...
            // new codes. If user retaps "Regenerate" the server will reject the old
            // TOTP code and the flow is harmless, but a second successful regenerate
            // would silently invalidate codes generated by the first request.
            // Retapping "Regenerate" after a cancel should be treated with caution.
            return
        } catch {
            errorMessage = AppError.from(error).errorDescription
        }
