...
/// Minimum-surface APIClient stub. Only the two inventory endpoints touched
/// by the view-model tests have canned answers; the rest throw to catch
/// accidental extra calls. Mirrors the pattern in
/// `Packages/Customers/Tests/CustomersTests/StubAPIClient.swift`.
actor StubAPIClient: APIClient {
    ...
    func post<T: Decodable & Sendable, B: Encodable & Sendable>(_ path: String, body: B, as type: T.Type) async throws -> T {
        guard path.hasPrefix("/api/v1/inventory"), let result = createResult else {
            throw APITransportError.noBaseURL
        }
        switch result {
    ...
    func put<T: Decodable & Sendable, B: Encodable & Sendable>(_ path: String, body: B, as type: T.Type) async throws -> T {
        guard path.hasPrefix("/api/v1/inventory/"), let result = updateResult else {
            throw APITransportError.noBaseURL
        }
        switch result {
