...
## Discovered — cross-agent dependency notes

- **2026-04-26 (Agent 8 / bef1335b) — `NSFaceIDUsageDescription` for §2.6 biometric login:** `ios/scripts/write-info-plist.sh` is owned by Agent 11 (tooling). That script already adds `NSMicrophoneUsageDescription` and `NSLocationWhenInUseUsageDescription` (update 37). The key `NSFaceIDUsageDescription = "Unlock BizarreCRM with Face ID"` is required by §2.6 but was not added — Agent 8 does not own `write-info-plist.sh` or `ios/App/Resources/Info.plist`. **Action for Agent 11 (tooling):** add `NSFaceIDUsageDescription` to `write-info-plist.sh` + `Info.plist`. Until then, any device build that calls `LAContext.evaluatePolicy(.deviceOwnerAuthenticationWithBiometrics)` will crash on first Face ID prompt.
