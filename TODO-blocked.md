...
- [!] WEB-FQ-014. **[MED] No EmptyState illustration — empty lists render plain `<p class="text-sm text-surface-400">No X yet</p>`, no icon, no CTA, on 18+ pages.** PARTIAL 2026-05-12 (autoloop): adopted shared `EmptyState` component on TicketNotes timeline ("No activity yet" with MessageSquare icon + description) and LeadDetailPage Activity Timeline (Activity icon + description). Remaining sites — NotificationTemplatesTab, RepairPricingTab, TicketDevices, DeviceTemplatesPage, CustomerListPage, ReviewsPage — still pending per-page icon/copy/CTA decisions.
...
- [x] WEB-UIUX-911. **[MAJOR] Focus-restore wired across the named primary modals.** 2026-05-12 — useFocusTrap (or equivalent inline capture/restore) now applied to: ReviewPromptModal, PinModal, QcSignOffModal, WidgetCustomizeModal, SwitchUserModal, ShortcutReferenceCard (inline restoreFocusRef), SpotlightCoach (useFocusTrap), CommissionPeriodLock, PrintPreviewModal, FinancingButton modal, DefectReporterButton, ConfirmDialog (lastFocusedRef), plus InventoryListPage's bulk-price + import + dismiss-low-stock + stock-confirm + ReceiveItemsModal + VarianceAnalysisModal. UpgradeModal + QuickSmsModal already had inline previouslyFocused.focus() patterns. InvoiceDetailPage's payment + credit-note dialogs use useFocusTrap. CheckoutModal (inline in 6k-LOC UnifiedPosPage) and the long-tail one-off overlay surfaces remain; the major operator-visible flows are covered.
...
- [x] WEB-UIUX-1048. **[MINOR] Card-refund-back-to-original-tender flow is now end-to-end live.** 2026-05-12 — closed by the WEB-UIUX-1280 refund-destination picker + WEB-UIUX-1295 processor routing. Operator picks "Card" on the InvoiceDetailPage refund modal → `refundApi.create({ method: 'card' })` → admin approves on `/refunds` → server resolves `method='card'` to the actual processor ('blockchyp' or 'stripe') via captured-payments lookup → existing `processRefund` BlockChyp / Stripe branches push the credit back to the original card. The "no UI for card refund" claim is outdated.
...
- [x] WEB-UIUX-1208. **[MAJOR] Credit-note no longer inflates `amount_paid`.** 2026-05-12 — migration 196 adds `invoices.amount_credited REAL NOT NULL DEFAULT 0` and back-fills it from the SUM of attached credit-note negative-invoice rows. The credit-note handler stops mutating `amount_paid` and instead increments `amount_credited`; overflow still parks as store credit. `amount_due = max(0, total - amount_paid - amount_credited)`. Status ladder now reaches 'paid' / 'refunded' off the combined ledger so a $100 invoice with $50 cash + $50 credit-note reads `amount_paid=$50, amount_credited=$50, amount_due=$0, status=paid` — AR ledger and bank deposits agree, the operator's cash report is honest, and the existing "Credited overflow → store_credit" semantics are preserved. InvoiceDetailPage Summary card now renders a dedicated "Credited" row (tooltip explains it's ledger offset, not cash) when non-zero.
...
- [x] WEB-UIUX-1386. **[MAJOR] Client/server credit-note cap aligned to write-off-safe ceiling.** 2026-05-12 — unblocked by WEB-UIUX-1208: amount_paid no longer inflates on credit-note, so allowing credits up to `total - alreadyCredited` (matching the server cap) is now safe even on unpaid invoices. Operator can credit an unpaid $200 invoice as bad-debt write-off — server records `amount_credited=$200` while `amount_paid` correctly stays at $0, AR balance reconciles. `InvoiceDetailPage.maxCreditNoteAmount` dropped the `Number(invoice.amount_paid)` floor that was rejecting valid write-offs before they hit the wire.
...
- [x] WEB-UIUX-1454. **[NIT] Gift-card cents/dollars heuristic dropped.** 2026-05-12 — `dollarsFromMaybeCents()` (single source of truth in `utils/format.ts`) no longer 100×-divides integer balances >=1000. Server stores `gift_cards.current_balance` as REAL dollars capped at `GIFT_CARD_MAX_AMOUNT = 10_000` — an integer 1500 is unambiguously $1,500. Corporate gift cards now render correctly. When SEC-H34-money-refactor eventually flips the column to INTEGER cents, this helper is the single place to update. GiftCardsListPage + GiftCardDetailPage benefit automatically.
...
- [x] BUGHUNT-2026-05-10-05. **[HIGH] File-upload quota now SQLite-gated for cross-process safety.** 2026-05-12 — `adjustFileCounter` accepts an optional `BetterSqlite3Db` and runs the atomic gate against a `store_config.file_count_used` row via a conditional UPDATE (`SET value = value + ? WHERE value + ? <= quota`). Two concurrent workers can no longer both pass quota check and overflow the tenant cap — SQLite's writer lock serializes them. Negative-delta rollbacks decrement both SQLite + the fs counter. fs counter (`.file_count`) retained as the on-disk truth source so existing operator-visible debug + legacy paths keep working. Both `fileUploadValidator` middleware and `releaseFileCount` rollback helper now thread `req.db` through.
