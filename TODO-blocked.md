...
- [!] WEB-UIUX-911. **[MAJOR] 30+ `role="dialog"` sites lack focus-restore on close.** Only ConfirmDialog implements lastFocused capture/restore. PinModal, UpgradeModal, QuickSmsModal, CheckoutModal, WidgetCustomizeModal, SwitchUserModal, ReviewPromptModal, 5 InventoryListPage modals — focus drops to body. L12. **PARTIAL 2026-05-12 (autoloop): `ReviewPromptModal` now calls `useFocusTrap(open)` on the dialog panel — focus traps inside the modal while open and restores to the post-pickup CTA trigger on close. Remaining list (PinModal / UpgradeModal / QuickSmsModal / CheckoutModal / WidgetCustomizeModal / SwitchUserModal / InventoryListPage modals + the long tail) still pending; same one-line hook call per site.**
