...
- [!] WEB-UIUX-911. **[MAJOR] 30+ `role="dialog"` sites lack focus-restore on close.** Only ConfirmDialog implements lastFocused capture/restore. PinModal, UpgradeModal, QuickSmsModal, CheckoutModal, WidgetCustomizeModal, SwitchUserModal, ReviewPromptModal, 5 InventoryListPage modals — focus drops to body. L12. **PARTIAL 2026-05-12 (autoloop): focus-restore wired in `ReviewPromptModal`, `PinModal`, `QcSignOffModal`, InventoryListPage's bulk-price + import + dismiss-low-stock + stock-confirm modals, `WidgetCustomizeModal` (DashboardPage), and `SwitchUserModal` (Header). `UpgradeModal` + `QuickSmsModal` already restore via inline `previouslyFocused.focus()` patterns. Remaining sites: CheckoutModal (inline in 6k-LOC UnifiedPosPage) + InvoiceDetailPage's payment+credit-note dialogs (those use `useFocusTrap` already as `paymentDialogRef`/`creditNoteDialogRef`) + low-stock-config + ItemMovementHistory + long tail. Most named primary modals done.
